  ...
  <div class="container">
    ...
    <div class="section">
      <div class="section-content">
        Please find attached your survey proposal for <strong>${proposal.propertyAddress}</strong>.
      </div>
    </div>
    ...
    if (emailResponse.error) {
      ...
      return NextResponse.json(
        { error: "Failed to send email", detail: emailResponse.error.message || JSON.stringify(emailResponse.error) },
        { status: 500 }
      );
    }
    ...
    // Update linked lead status if applicable
    if (proposal.leadId) {
      await db
        .update(leads)
        .set({
